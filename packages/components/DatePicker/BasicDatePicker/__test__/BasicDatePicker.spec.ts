...
describe('BasicDatePicker.vue',() => {
  test("render",async () => {
    // 获取组件
    const wrapper = mount(BasicDatePicker);
    
    // 刚开始选择框没有出现
    // expect(wrapper.find(".wv-DatePicker-content-left").isVisible()).toBe(true);
    ...
    expect(wrapper.find(".wv-DatePicker").attributes("style")).toBe("position: absolute; top: -10px; left: 0px;");
    // 获取今日日期
    let date = new Date();
