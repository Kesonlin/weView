...
import WvDropdown from './dropdown/index.vue'
import WvEmpty from './empty/index.vue'
...
const sortTable = (e: any, key: string, sort: string) => {
  let currentSort = e.target.parentNode.getAttribute('wv-sort')
  if (sort === 'desc') {
    if (currentSort === sort) {
      e.target.parentNode.setAttribute('wv-sort', '')
      tableDataSource.value = [...props.dataSource]
    } else {
      e.target.parentNode.setAttribute('wv-sort', 'desc')
    ...
    if (currentSort === sort) {
      ...
      e.target.parentNode.setAttribute('wv-sort', 'asc')
      tableDataSource.value.sort((x, y) => {
        if (x[key] < y[key]) return -1
        else if (x[key] > y[key]) return 1
        else return 0
...
const classes = computed(() => {
  return [
    hasl.value ? 'wv-table-has-fixed-left' : '',
    hasr.value ? 'wv-table-has-fixed-right' : '',
  ]
})
...
const renderHeadFixedStyle = (
  ...
  if (column.fixed) {
    ...
    if (tableHeadColumnIndex == 0) {
      let isLast = true
      for (let i = columnIndex + 1; i < tableHeadColumn.length; i++) {
        if (
      ...
      let isLast = true
      // 父节点是否为当前层级的尾节点。
      for (let i = index + 1; i < topicColumns.length; i++) {
        if (
